...
\textbf{Grupo} nr. 39
\\\hline
a97768 & Breno Fernando G. Marrão
\\
a96314 & Tales Andre M. Rovaris
\\
a96414 & Tiago Passos Rodrigues
\end{tabular}
\end{center}
...
Aplicando a lei da recursividade mútua:

\begin{code}
q d 0 = 0
q d (n+1) = h1 . (q d (r d, c d))
\end{code}
...
\begin{code}
r d 0 = 0
r d (n+1) = h2 . (q d (r d, c d))
\end{code}

Concluimos então que:

\begin{code}
r d (n+1) = if (snd.snd) == 0 then 0 else (+) 1 (fst.snd)
\end{code}

\begin{code}
c d 0 = d
c d (n+1) = h3 . (q d (r d, c d))
\end{code}

\begin{code}
c d (n+1) = if (snd.snd) == 0 then d else (-) (snd.snd) 1
\end{code}

\begin{eqnarray*}
\start
     |cata (split (either 0 h1) (split (either 0 h2) (either d h3)))|
\just\equiv{ (28) }
     |cata (split (either 0 h1) (either (split 0 d) (split h2 h3)))|
\just\equiv{ (28) }
     |cata (either (split 0 (split 0 d)) (split h1 (split h2 h3)))|
